...
SELECT
    ...
    NULLIF(COALESCE(p.precio_producto, 0),0) * NULLIF(COALESCE(v.cantidad_pedido, 0),0) AS ingreso_total,
    NULLIF(COALESCE(p.costo_producto, 0),0) * NULLIF(COALESCE(v.cantidad_pedido, 0),0) AS costo_total
